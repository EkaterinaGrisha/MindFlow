/**
 * Заглушка разборов задач (worked-rich) для темы «Градиентный спуск — часть 2».
 * Отображается, пока соответствующий блок практики находится в плане
 * ближайшего расширения каталога. Полноценные разборы появляются по мере
 * доведения тем до уровня расширенной интерактивной теории.
 */

export default function GradientDescentPart2WorkedRich(_: {
  onGoTheory?: () => void;
  onGoPractice?: () => void;
}) {
  return (
    <div style={{ padding: 24, fontFamily: "'Inter', sans-serif", color: '#64748b' }}>
      Разборы задач для темы «Градиентный спуск — часть 2» появятся в одном из ближайших обновлений каталога.
    </div>
  );
}
